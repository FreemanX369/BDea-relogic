//+------------------------------------------------------------------+
//| BlackDragon.mq5 — EA Black Dragon v15.04 / T17.27 source candidate |
...
#include <BlackDragon/ConfigDiagnosticsT1727.mqh>
...
int OnInit()
{
   ...
   Log_EffectiveConfigT1727();
   Log_Info("Init", "EA Black Dragon v15.04 T17.27 source candidate started. ExecMode=" +
...
void OnDeinit(const int reason)
{
   ...
   Print("T17.25 observation scans=",g_bdObservationBook.Scans(),
...
void OnTick()
{
   CBDMetricScope metricScope(BD_M_TICK);
   EAContext ctx;
   if(!BuildContext(ctx)) return;
   {CBDMetricScope scope(BD_M_SIGNAL);g_signal.Compute(ctx);}
   ...
   {CBDMetricScope scope(BD_M_BASKET);g_basket.Update(ctx);}
   {CBDMetricScope scope(BD_M_PROTECTION);g_pyProtection.Observe(ctx);}
   ...
   {CBDMetricScope scope(BD_M_RECOVERY);g_recovery.OnTick(ctx);}
   g_bdObservationBook.End(); // all mutation paths below continue to read live state
   {CBDMetricScope scope(BD_M_STRATEGY);g_strategy.OnTick(ctx);}
}

//+------------------------------------------------------------------+
void OnTimer()
{
   CBDMetricScope metricScope(BD_M_TIMER);
   g_news.Refresh();
   {CBDMetricScope scope(BD_M_EXECUTOR);g_exec.Watchdog();}
   g_recovery.FlushPersistence();
   string exitWhy = "";
...
{
   CBDMetricScope metricScope(BD_M_TRANSACTION);
   g_bdObservationBook.Invalidate();
      ...
      trans.type==TRADE_TRANSACTION_DEAL_UPDATE || trans.type==TRADE_TRANSACTION_DEAL_DELETE)
   {
      ...
      Pyramid_ObserveCampaignTransaction(trans);
      g_basket.Invalidate();
   }
   ...
   {CBDMetricScope scope(BD_M_EXECUTOR);g_exec.OnTransaction(trans, request, result);}
   if(pyTopology)
