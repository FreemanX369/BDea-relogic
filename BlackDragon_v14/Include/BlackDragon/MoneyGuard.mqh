...
eGuardAction MG_LatchNextPure(const eGuardAction latched,const eGuardAction triggered,
                              const bool scopeFlat)
{
   if(scopeFlat || latched==GUARD_NONE)return triggered;
   if(latched==GUARD_CLOSE_ACCOUNT || triggered==GUARD_CLOSE_ACCOUNT)return GUARD_CLOSE_ACCOUNT;
   if(latched==GUARD_CLOSE_MAGIC_DAILY || triggered==GUARD_CLOSE_MAGIC_DAILY)return GUARD_CLOSE_MAGIC_DAILY;
   if(latched==GUARD_CLOSE_MAGIC || triggered==GUARD_CLOSE_MAGIC)return GUARD_CLOSE_MAGIC;
   if((latched==GUARD_CLOSE_BUY && triggered==GUARD_CLOSE_SELL) ||
      (latched==GUARD_CLOSE_SELL && triggered==GUARD_CLOSE_BUY))return GUARD_CLOSE_MAGIC;
   return latched;
}
