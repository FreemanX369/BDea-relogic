...
private:
   ...
   CDeferredExitStoreT1727 m_deferredExits;
                        ...
                        const SRecoveryT165GuardMetrics &rg)
   {
      ...
      if(!m_deferredExits.SetGuard((int)m_guardLatched))return true;
      m_exec.ReconcileAll();
      ...
      if(flat && !pendingOpen && !pendingClose && !recoveryBusy && !m_exec.HasPendingMutation())
      {
         ...
         if(!m_deferredExits.SetGuard((int)GUARD_NONE))return true;
      ...
      if(pendingClose || m_exec.HasPendingMutation())
         return true;
   ...
   bool ApplyGuardPriority(const EAContext &ctx)
   {
      ...
      eGuardAction action = m_guard.CheckFloatingPriority(ctx.now,
                                                          ...
                                                          bothCoreOpen,
                                                          accountFloating);
      if(action == GUARD_NONE && m_guardLatched == GUARD_NONE) return false;
