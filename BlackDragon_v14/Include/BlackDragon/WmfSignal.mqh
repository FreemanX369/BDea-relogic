...
private:
   ...
   bool CandleValidT1727(const MqlRates &r,const double atrRaw) const
   {
      return MathIsValidNumber(r.open)&&r.open!=EMPTY_VALUE&&
         MathIsValidNumber(r.high)&&r.high!=EMPTY_VALUE&&
         MathIsValidNumber(r.low)&&r.low!=EMPTY_VALUE&&
         MathIsValidNumber(r.close)&&r.close!=EMPTY_VALUE&&r.high>=r.low&&
         MathIsValidNumber(atrRaw)&&MathIsValidNumber(AtrOf(atrRaw,r.high,r.low));
   }
   ...
   bool Seed()
   {
      ...
      for(int i = 0; i < got; i++)
      {
         if(!CandleValidT1727(rates[i],atr[i]))return false;
      ...
      if(m_barSignal != ctx.barTime)
      {
         ...
         if(newest != m_lastClosed)
         {
            ...
            else
            {
               ...
               if(!CandleValidT1727(r[0],atr[0])){ Publish(ctx); return; }
         ...
         if(Use_Stoh)
         {
            ...
            if(!MathIsValidNumber(stochBuf[0]) || stochBuf[0]==EMPTY_VALUE || stochBuf[0]<0.0 || stochBuf[0]>100.0)
            { Publish(ctx); return; }
            d = stochBuf[0];
         }
