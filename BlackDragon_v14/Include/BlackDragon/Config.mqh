...
input group "G01 — BẢO VỆ LỢI NHUẬN NHÓM PYRAMID CORE"
...
input group "G02 — Vận hành chung & định danh EA"
...
input group "G03 — Nguồn tín hiệu & Black Dragon"
...
input group "G04 — Xác nhận Stochastic"
...
input group "G06 — Quản lý Lot & giới hạn rổ"
...
input group "G07 — Khoảng cách DCA"
...
input group "G08 — TP / SL / Trailing / Overlap"
...
input group "G09 — Chốt lời/lỗ theo tiền"
...
input group "G10 — Mục tiêu & giới hạn theo ngày"
...
input group "G11 — Bộ lọc mở chuỗi mới"
input double    MaxSpred     = 0;     // Legacy: point chuẩn; PIP_UNIFIED: pip; 0 = tắt
...
input group "G12 — Lịch giao dịch theo giờ máy tính"
...
input group "G13 — Bộ lọc tin tức"
...
input group "G14 — Khớp lệnh & chuẩn hóa broker"
                            ...
                            const double tickSize, string &why)
{
   ...
   if(!Unit_BuildProfilePure(isGold, point, digits, tickSize, AutoGoldPip, p, why))
      return false;
   if(UnitSystemMode_!=unit_LEGACY_COMPAT && UnitSystemMode_!=unit_PIP_UNIFIED)
   {why="invalid UnitSystemMode_";return false;}
   if(!Unit_FinitePure(TP_)||TP_<0 || !Unit_FinitePure(SL_)||SL_<0 ||
      !Unit_FinitePure(iTS)||iTS<0 || !Unit_FinitePure(iTD)||iTD<0 ||
      !Unit_FinitePure(MaxSpred)||MaxSpred<0 || !Unit_FinitePure(Slippage_)||Slippage_<0 ||
      !Unit_FinitePure(MaxLot_)||MaxLot_<0)
   {why="invalid distance/volume input";return false;}
   if(!Unit_FinitePure(PyramidBETriggerPips_) ||
      !Unit_FinitePure(PyramidLockProfitPips_) ||
      !Unit_FinitePure(PyramidLockSafetyPips_) ||
      !Unit_FinitePure(PyramidTrailGapPips_) ||
      !Unit_FinitePure(WmfFactor) ||
      !Unit_FinitePure(Lot_Init_) ||
      !Unit_FinitePure(MaxLot_) ||
      !Unit_FinitePure(TP_) ||
      !Unit_FinitePure(SL_) ||
      !Unit_FinitePure(iTS) ||
      !Unit_FinitePure(iTD) ||
      !Unit_FinitePure(OverlapPercent) ||
      !Unit_FinitePure(PctDiffClose) ||
      !Unit_FinitePure(MoneyTPAllAccount) ||
      !Unit_FinitePure(MoneySLAllAccount) ||
      !Unit_FinitePure(MoneyTPAll) ||
      !Unit_FinitePure(MoneySLAll) ||
      !Unit_FinitePure(MoneyTPBuy) ||
      !Unit_FinitePure(MoneySLBuy) ||
      !Unit_FinitePure(MoneyTPSell) ||
      !Unit_FinitePure(MoneySLSell) ||
      !Unit_FinitePure(MoneyTPAllHedged) ||
      !Unit_FinitePure(DailyTPMoney) ||
      !Unit_FinitePure(DailySLMoney) ||
      !Unit_FinitePure(DailyTPPercent) ||
      !Unit_FinitePure(DailySLPercent) ||
      !Unit_FinitePure(MaxSpred) ||
      !Unit_FinitePure(MinAdx) ||
      !Unit_FinitePure(Slippage_))
   {why="nonfinite numeric input";return false;}
   if((ExecMode!=exec_Sync&&ExecMode!=exec_Async) ||
      (TP_Mode!=mode_Real&&TP_Mode!=mode_Virt) || (SL_Mode!=mode_Real&&SL_Mode!=mode_Virt) ||
      (Trail_Mode!=mode_Real&&Trail_Mode!=mode_Virt) ||
      (SignalSource_!=sig_BD&&SignalSource_!=sig_WMF) ||
      (LotMode_!=lot_Sequence&&LotMode_!=lot_MultiplierChain) ||
      (NewsFailMode!=news_fail_TradeOn&&NewsFailMode!=news_fail_PauseNew) ||
      (Autolot_&&Autolotsize_<=0) || (Use_Stoh&&(KPeriod<=0||DPeriod<=0||Slowing<=0||Down_Level<0||Up_Level>100||Down_Level>Up_Level)))
   {why="invalid active enum/range";return false;}
   Cfg.TP=TP_;Cfg.SL=SL_;Cfg.TrailStart=iTS;Cfg.TrailDistance=iTD;
   ...
   if(!Unit_FinitePure(Cfg.TPPrice)||!Unit_FinitePure(Cfg.SLPrice)||
      !Unit_FinitePure(Cfg.TrailStartPrice)||!Unit_FinitePure(Cfg.TrailDistancePrice)||
      !Unit_FinitePure(Cfg.MaxSpreadPrice)||!Unit_FinitePure(Cfg.SlippagePrice)||
      !Unit_PositivePure(Cfg.DcaInputUnitPrice)||Cfg.SlippagePrice/p.point>9007199254740991.0)
   {why="canonical distance overflow";return false;}
   return true;
}
#endif // BD_CONFIG_MQH
