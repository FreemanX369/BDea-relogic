...
private:
   ...
   bool DriveIndependentExitSideT1727(const EAContext &ctx,const BasketSide &side,const int dir)
   {
      int pending=m_deferredExits.Kind(dir);
      bool openPending=m_exec.BusyOpen(dir) || (m_pyramid!=NULL&&m_pyramid.HasPending(dir));
      if(side.count==0&&!openPending&&!RecoveryOwnsExitSideT1712(dir))
      {if(pending!=EXIT_NONE){m_deferredExits.Set(dir,EXIT_NONE);return true;}return false;}
      ExitDecision d=m_exitPolicy.Check(ctx,side,dir);
      bool slHit=SL_Mode==mode_Virt && Exit_VirtualSlHit(dir==BD_DIR_BUY,side.slLevel,ctx.bid,ctx.ask);
      if(slHit)d.kind=EXIT_SL;
      if(pending==EXIT_NONE || (slHit&&pending!=EXIT_SL))
      {
         if(d.kind!=EXIT_TP&&d.kind!=EXIT_SL&&d.kind!=EXIT_TRAIL)return false;
         if(d.kind==EXIT_TP&&m_pyramid!=NULL)
         {
            double target=side.tpLevel;
            if(!m_pyramid.EconomicTpLevel(side,dir,side.tpLevel,target)||!Exit_VirtualTpHit(dir==BD_DIR_BUY,target,ctx.bid,ctx.ask))return false;
         }
         if((d.kind==EXIT_TP||d.kind==EXIT_TRAIL)&&!RecoveryExitFundedT1712(ctx,side,dir,d.kind))return false;
         if(!m_deferredExits.Set(dir,(int)d.kind))return true;
         pending=(int)d.kind;
      }
      if(pending==EXIT_NONE)return false;
      m_exec.ReconcileAll();
      if(g_pyramidProtection!=NULL)g_pyramidProtection.SettleForIndependentExitT1727();
      if(m_exec.HasPendingMutation()||openPending)return true;
      if((pending==EXIT_TP||pending==EXIT_TRAIL)&&!RecoveryExitFundedT1712(ctx,side,dir,(eExitKind)pending))return true;
      eRecoveryExitCoordReason reason=pending==EXIT_TP?recovery_EXIT_REASON_LEGACY_TP:
         pending==EXIT_SL?recovery_EXIT_REASON_LEGACY_SL:recovery_EXIT_REASON_LEGACY_TRAIL;
      eRecoveryExitCoordRequest request=BeginFullSideClose(dir,reason,ctx.now);
      if(request==recovery_EXIT_BYPASS){m_exec.CloseBasket(side);m_basket.Invalidate();}
      else if(request!=recovery_EXIT_BLOCKED)DriveRecoveryExit(ctx.now);
      return true;
   }
   bool DriveIndependentExitsT1727(const EAContext &ctx)
   {
      RefreshPyramidCampaigns(ctx);
      if(ApplyGuardSecondary(ctx))return true;
      if(!m_deferredExits.Healthy())return true;
      bool buy=DriveIndependentExitSideT1727(ctx,m_basket.buy,BD_DIR_BUY);
      bool sell=DriveIndependentExitSideT1727(ctx,m_basket.sell,BD_DIR_SELL);
      return buy||sell;
   }
...
public:
             ...
             CCorePyramidEngine *pyramid)
   {
      ...
      if(!m_deferredExits.Init(!MQLInfoInteger(MQL_TESTER)||RecoveryTesterResumeState_))
         Log_Error("Strategy","T17.27 deferred exit checkpoint invalid; new growth blocked");
      else m_guardLatched=(eGuardAction)m_deferredExits.Guard();
      string overlapWhy = "";
   ...
   void OnTick(const EAContext &ctx)
   {
      ...
      if(g_pyramidProtection!=NULL)g_pyramidProtection.SetExitOverride(true);
      bool independentExit=DriveIndependentExitsT1727(ctx);
      if(g_pyramidProtection!=NULL)g_pyramidProtection.SetExitOverride(false);
      if(independentExit)return;
      ...
      // T17.27: daily/virtual intent evaluated before the PY wait boundary.
