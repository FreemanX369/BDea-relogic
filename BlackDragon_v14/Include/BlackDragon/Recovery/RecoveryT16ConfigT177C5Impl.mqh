...
input group "G26 — T17.7: CẤU HÌNH GỘP / TƯƠNG THÍCH"
...
input group "G27 — T17.19: TÁI VÀO RECOVERY SAU BE/SL DƯƠNG"
...
uint Recovery_T177SemanticFingerprintC5()
{
   ...
   string canonical =
      "t1727UnitRev=" + (string)BD_UNIT_POLICY_REV +
      "|unitMode=" + (string)(int)Cfg.UnitMode +
      "|point=" + DoubleToString(Cfg.Point,16) +
      "|tick=" + DoubleToString(Cfg.TickSize,16) +
      "|pip=" + DoubleToString(Cfg.PipSize,16) +
      "|legacyUnit=" + DoubleToString(Cfg.LegacyPointSize,16) +
      "|dcaUnit=" + DoubleToString(Cfg.DcaInputUnitPrice,16) +
      "|slippagePrice=" + DoubleToString(Cfg.SlippagePrice,16) +
   ...
   if(CorePyramidMode_ != pyramid_TAT)
   {
      canonical +=
         ...
         "|coreLotSeq=" + (PyramidLotMode_==pyramid_LOT_CHUOI?PyramidLotSequence_:"") +
         "|coreMulSeq=" + (PyramidLotMode_==pyramid_LOT_HE_SO?PyramidMultiplierSequence_:"") +
