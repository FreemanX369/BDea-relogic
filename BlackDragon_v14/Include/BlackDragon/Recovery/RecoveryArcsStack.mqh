...
#include "RecoveryArcsLedger.mqh"
...
private:
   ...
   CArcsLedgerPersistence m_persistence;
   bool   m_initialized;
   bool   m_ready;
   ...
   {
      int i = Idx(dir);
      g_bdMetrics.Add(BD_M_RECONCILE);
   ...
   {
      if(deal == 0 || !HistoryDealSelect(deal)) return -1;
   ...
   int ReadReceipt(const ulong deal,const bool liveEvent,SBDArcsReceipt &r,string &why)
   {
      ZeroMemory(r);why="";
      if(!HistoryDealSelect(deal)){why="deal unavailable";return -1;}
      if(HistoryDealGetString(deal,DEAL_SYMBOL)!=_Symbol)return 0;
      long entry=HistoryDealGetInteger(deal,DEAL_ENTRY);
      if(entry!=DEAL_ENTRY_OUT&&entry!=DEAL_ENTRY_OUT_BY)return 0;
      r.deal=deal;r.positionId=(ulong)HistoryDealGetInteger(deal,DEAL_POSITION_ID);r.stamp=HistoryDealGetInteger(deal,DEAL_TIME_MSC);
      r.units=Recovery_VolumeToUnitsFloor(HistoryDealGetDouble(deal,DEAL_VOLUME),m_volumeStep);
      r.programmedSl=HistoryDealGetDouble(deal,DEAL_SL);
      long closeReason=HistoryDealGetInteger(deal,DEAL_REASON);
      r.cash=Recovery_DealCashPure(HistoryDealGetDouble(deal,DEAL_PROFIT),HistoryDealGetDouble(deal,DEAL_SWAP),HistoryDealGetDouble(deal,DEAL_COMMISSION),HistoryDealGetDouble(deal,DEAL_FEE));
      long type=HistoryDealGetInteger(deal,DEAL_TYPE);r.owner=ResolveClosedOwnerMagic(deal);
      if(r.owner<0){why="opening ownership unavailable";return -1;}
      if(r.owner!=(long)Magic&&r.owner!=(long)RecoveryMagic_)return 0;
      bool mapped=false;eRecoveryCoreDirection direction=DirectionForClose(r.owner,type,mapped);
      if(!mapped){why="close direction unavailable";return -1;}r.dir=Idx(direction);
      r.generation=r.owner==(long)RecoveryMagic_?Recovery_ArcsGenerationFromPositionHistory(r.positionId):0;
      if(r.owner==(long)RecoveryMagic_&&r.generation<1){why="generation unavailable";return -1;}
      SBDArcsReceipt old;bool known=m_persistence.Get(deal,old);
      if(!known&&!liveEvent&&m_persistence.BeforeFreshStart(r.dir,r.stamp))return 0;
      if(!known&&m_persistence.LegacyCovered(r.dir,r.stamp,deal))
      {if(!liveEvent)return 0;why="event precedes migrated receipt coverage";return -1;}
      r.pyTrim=g_pyramidProtection!=NULL&&g_pyramidProtection.ExpectedRhTrim(deal);
      if(known&&old.positionId==r.positionId&&old.owner==r.owner&&old.dir==r.dir&&old.generation==r.generation&&old.stamp==r.stamp)
      {r.cycle=old.cycle;r.phase=old.phase;r.funding=old.funding;r.coreFunding=old.coreFunding;r.pyTrim=old.pyTrim;r.suppressed=old.suppressed;r.adjustBaseline=old.adjustBaseline;
       r.protectiveSl=old.protectiveSl&&closeReason==DEAL_REASON_SL&&
                      MathAbs(old.programmedSl-r.programmedSl)<=1e-12;}
      else
      {
         SBDArcsEpoch epoch;if(!m_persistence.PhaseAt(r.dir,r.stamp,epoch)){why="funding phase proof unavailable/ambiguous";return -1;}
         r.cycle=epoch.cycle;r.phase=epoch.phase;r.coreFunding=r.owner==(long)Magic&&epoch.phase==ARCS_CORE_FUNDING;
         r.funding=r.owner==(long)RecoveryMagic_&&!r.pyTrim&&epoch.phase==ARCS_TP_PENDING&&epoch.generation==r.generation;
         r.adjustBaseline=r.pyTrim&&epoch.phase==ARCS_CORE_FUNDING&&epoch.generation==r.generation;
      }
      if(!BD_ReceiptValid(r)){why="invalid receipt";return -1;}return 1;
   }
   bool ApplyReceiptEffect(const SBDArcsReceipt &r,const int sign,string &why)
   {
      if(r.deleted||r.suppressed||r.cycle!=m_persistence.Cycle(r.dir))return true;
      int d=r.dir;eRecoveryCoreDirection dir=d==0?recovery_CORE_BUY:recovery_CORE_SELL;
      if(r.owner==(long)Magic)m_dir[d].coreLossSpent+=sign*BD_ReceiptCoreLoss(r);
      else
      {
         int li=FindLayerByGeneration(dir,r.generation);if(li<0){why="receipt references missing layer";return false;}
         SArcsLayer layer;GetLayer(dir,li,layer);
         if(r.funding){layer.fundingClosedUnits+=sign*r.units;layer.realizedFundingCash+=sign*r.cash;m_dir[d].hedgeFundingCash+=sign*r.cash;}
         else layer.realizedOtherCash+=sign*r.cash;
         if(r.pyTrim)
         {
            if(r.adjustBaseline)layer.tpBaselineUnits-=sign*r.units;
            if(sign>0)
            {
               layer.remainingUnits=Recovery_ArcsLayerUnits(dir,r.generation,m_volumeStep);
               if(layer.remainingUnits==0){layer.state=ARCS_LAYER_CLOSED;layer.virtualSlArmed=false;if(m_dir[d].activeLayer==li)m_dir[d].phase=ARCS_LOCKED;}
            }
         }
         if(layer.fundingClosedUnits<0||layer.tpBaselineUnits<0){why="receipt correction unit underflow";return false;}
         PutLayer(dir,li,layer);
      }
      if(m_dir[d].coreLossSpent<-1e-8){why="cash correction underflow";return false;}
      if(m_dir[d].coreLossSpent<0)m_dir[d].coreLossSpent=0;
      Recovery_ArcsRecomputeCredit(m_dir[d]);m_dirty=true;return true;
   }
   bool ReconcileReceipt(const ulong deal,const bool deleted,const bool liveEvent,string &why)
   {
      SBDArcsReceipt old,next;ZeroMemory(old);bool known=m_persistence.Get(deal,old);
      if(deleted)
      {if(!known){why="delete outside retained receipt proof";m_persistenceBlocked=true;return false;}next=old;next.deleted=true;}
      else
      {int state=ReadReceipt(deal,liveEvent,next,why);if(state<0){m_persistenceBlocked=true;return false;}if(state==0){if(!known)return true;next=old;next.deleted=true;}}
      if(known&&BD_ReceiptEqual(old,next))return true;
      // T17.27: constant-size rollback, not a copy of the complete layer arrays.
      SArcsDirection beforeBuy=m_dir[0],beforeSell=m_dir[1];
      bool beforeDirty=m_dirty;
      eRecoveryCoreDirection oldDir=old.dir==0?recovery_CORE_BUY:recovery_CORE_SELL;
      eRecoveryCoreDirection nextDir=next.dir==0?recovery_CORE_BUY:recovery_CORE_SELL;
      int oldIndex=known&&old.owner==(long)RecoveryMagic_?FindLayerByGeneration(oldDir,old.generation):-1;
      int nextIndex=next.owner==(long)RecoveryMagic_?FindLayerByGeneration(nextDir,next.generation):-1;
      SArcsLayer beforeOld,beforeNext;ZeroMemory(beforeOld);ZeroMemory(beforeNext);
      if(oldIndex>=0)GetLayer(oldDir,oldIndex,beforeOld);
      if(nextIndex>=0)GetLayer(nextDir,nextIndex,beforeNext);
      bool ok=!known||ApplyReceiptEffect(old,-1,why);
      if(ok)ok=ApplyReceiptEffect(next,1,why);
      if(ok&&!m_persistence.Put(next)){why="receipt capacity/validation";ok=false;}
      if(!ok)
      {
         m_dir[0]=beforeBuy;m_dir[1]=beforeSell;
         if(oldIndex>=0)PutLayer(oldDir,oldIndex,beforeOld);
         if(nextIndex>=0)PutLayer(nextDir,nextIndex,beforeNext);
         m_dirty=beforeDirty;m_ready=false;m_persistenceBlocked=true;
         return false;
      }
      if(!deleted)TrackCursor(nextDir,deal);
      m_dirty=true;return true;
   }
   bool ReplayAfterCursor(const eRecoveryCoreDirection dir,string &why)
   {
      why="";int di=Idx(dir);datetime end=TimeCurrent();
      if(!HistorySelect(m_persistence.ReplayFrom(di,m_dir[di].lastDealTimeMsc),end)){why="history unavailable";return false;}
      int total=HistoryDealsTotal();ulong candidates[];
      if(ArrayResize(candidates,total)!=total){why="replay allocation";return false;}
      for(int i=0;i<total;i++){candidates[i]=HistoryDealGetTicket(i);if(candidates[i]==0){why="replay ticket missing";return false;}}
      ulong replay[];long stamps[];
      for(int i=0;i<total;i++)
      {
         ulong deal=candidates[i];if(!HistoryDealSelect(deal)){why="deal unavailable";return false;}
         if(HistoryDealGetString(deal,DEAL_SYMBOL)!=_Symbol)continue;
         long tmsc=HistoryDealGetInteger(deal,DEAL_TIME_MSC),entry=HistoryDealGetInteger(deal,DEAL_ENTRY),type=HistoryDealGetInteger(deal,DEAL_TYPE);
         if(entry!=DEAL_ENTRY_OUT&&entry!=DEAL_ENTRY_OUT_BY)continue;
         if(m_persistence.BeforeFreshStart(di,tmsc))continue;
         long owner=ResolveClosedOwnerMagic(deal);if(owner<0){why="opening ownership unavailable";return false;}
         if(owner!=(long)Magic&&owner!=(long)RecoveryMagic_)continue;
         bool mapped=false;eRecoveryCoreDirection dealDir=DirectionForClose(owner,type,mapped);
         if(!mapped || dealDir!=dir) continue;
         int n=ArraySize(replay);
         if(ArrayResize(replay, n + 1,128)!=n+1||ArrayResize(stamps,n+1,128)!=n+1){why="batch allocation";return false;}
         replay[n]=deal;stamps[n]=tmsc;
      }
      int n=ArraySize(replay);ulong temp[];long tempStamp[];
      if(ArrayResize(temp,n)!=n||ArrayResize(tempStamp,n)!=n){why="merge allocation";return false;}
      g_bdMetrics.Add(BD_M_SORT_ITEMS,(ulong)n);
      for(int width=1;width<n;width*=2)
      {
         for(int left=0;left<n;left+=2*width)
         {
            int mid=(int)MathMin(left+width,n),endIndex=(int)MathMin(left+2*width,n),i=left,j=mid;
            for(int k=left;k<endIndex;k++)
            {bool take=i<mid&&(j>=endIndex||stamps[i]<stamps[j]||(stamps[i]==stamps[j]&&replay[i]<replay[j]));int at=take?i++:j++;temp[k]=replay[at];tempStamp[k]=stamps[at];}
         }
         for(int k=0;k<n;k++){replay[k]=temp[k];stamps[k]=tempStamp[k];}
      }
      for(int i=0;i<n;i++)if(!ReconcileReceipt(replay[i],false,false,why))return false;
      m_persistence.ScannedThrough(di,(long)end*1000);m_dirty=true;return true;
   }
   ...
   {
      int i = Idx(dir);
      if(m_dir[i].phase==ARCS_IDLE&&m_dir[i].anchorPosition==0&&m_dir[i].generationCount==0&&!m_pending[i].active)return;
      m_persistence.NewCycle(i);
   ...
   {
      m_persistence.NewCycle(Idx(dir));
      int di = Idx(dir);
...
public:
   ...
   bool Init()
   {
      ...
      g_bdArcsReceiptStore=GetPointer(m_persistence);
   ...
   bool RefreshCashReplayT1727(const EAContext &ctx)
   {
      if(!m_initialized || RecoveryMode_==recovery_OFF)return true;
      if(m_overlapAt>0&&ctx.now<m_overlapAt-30)m_overlapAt=ctx.now;
      if(RecoveryMode_==recovery_ACTIVE&&m_ready&&!m_persistenceBlocked&&ctx.now>=m_overlapAt)
      {
         m_overlapAt=ctx.now+30;string overlapWhy;
         if(!ReplayAfterCursor(recovery_CORE_BUY,overlapWhy)||!ReplayAfterCursor(recovery_CORE_SELL,overlapWhy))
         {m_persistenceBlocked=true;LatchReconcile(recovery_CORE_BUY,overlapWhy);LatchReconcile(recovery_CORE_SELL,overlapWhy);return false;}
         if(m_dirty&&!Save(overlapWhy))return false;
      }
   ...
   {
      if(!RefreshCashReplayT1727(ctx))return;
   ...
   {
      if(!m_initialized||RecoveryMode_!=recovery_ACTIVE||m_persistenceBlocked)return;
      if(trans.symbol!=_Symbol&&trans.symbol!="")return;
      if(trans.type!=TRADE_TRANSACTION_DEAL_ADD&&trans.type!=TRADE_TRANSACTION_DEAL_UPDATE&&trans.type!=TRADE_TRANSACTION_DEAL_DELETE)return;
      if(trans.deal==0)return;string why="";bool deleted=trans.type==TRADE_TRANSACTION_DEAL_DELETE;
      if(!ReconcileReceipt(trans.deal,deleted,true,why))
      {m_persistenceBlocked=true;LatchReconcile(recovery_CORE_BUY,"history correction requires funding-ledger reconciliation: "+why);LatchReconcile(recovery_CORE_SELL,why);return;}
      if(m_dirty&&!Save(why))Log_Error("Recovery","atomic ledger save failed: "+why);
   }

   bool FlushPersistence()
   {
      if(RecoveryMode_ != recovery_ACTIVE) return true;
      if(m_persistenceBlocked)return false;
   ...
   {
      SBDArcsReceipt r;string why="";int state=ReadReceipt(deal,false,r,why);
      if(state<0){m_persistenceBlocked=true;LatchReconcile(recovery_CORE_BUY,why);LatchReconcile(recovery_CORE_SELL,why);return;}
      if(state==0)return;SBDArcsReceipt old;if(m_persistence.Get(deal,old))return;
      r.suppressed=true;if(!m_persistence.Put(r)){m_persistenceBlocked=true;return;}
      TrackCursor(r.dir==0?recovery_CORE_BUY:recovery_CORE_SELL,deal);m_dirty=true;
   }
   ...
   void GetCycle(const eRecoveryCoreDirection dir, SRecoveryCycle &out) const
   {
      ...
      out.coreCount = Recovery_ArcsTriggerCoreCount(dir, m_volumeStep);
      long coreUnits = Recovery_ArcsCoreUnits(dir, m_volumeStep);
