...
double Pyramid_NormalizeFloor(double lot)
{
   ...
   // raw==cap preserves floor semantics while rejecting invalid/overflowing metadata.
   return Unit_CappedVolumePure(lot,lot,vMin,vMax,step);
}
...
#include "CampaignLedger.mqh"
bool Pyramid_ReadCampaignHistory(const int dir,const datetime campaignStartTime,const long campaignStartTimeMsc,const datetime now,SPyramidCampaignStats &stats)
{
   if(dir<0||dir>1||campaignStartTime<=0){Pyramid_CampaignReset(stats);return false;}
   return g_bdCampaignLedger[dir].Read(dir,campaignStartTimeMsc,now,g_pyramidDealRevision,stats);
}
...
private:
               ...
               CRecoveryEngine *recovery, string &why)
   {
      ...
      if(PyramidLotMode_ == pyramid_LOT_CHUOI)
      {
         ...
         candidate = Grid_NormalizeCoreVolume(candidate);
         if(candidate <= 0.0) return false;
