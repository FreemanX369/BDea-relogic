...
private:
   ...
   {
      return Recovery_StringHash("t1727|"+(string)BD_UNIT_POLICY_REV+"|"+
         (string)(int)Cfg.UnitMode+"|"+DoubleToString(Cfg.PipSize,16)+"|"+
         DoubleToString(Cfg.TickSize,16)+"|"+DoubleToString(Cfg.SlippagePrice,16)+"|"+
         DoubleToString(Recovery_T177EffectiveHedgeAbsoluteMaxCoveragePercent(),12)+"|"+
         (string)(int)PyramidSLMode_+"|"+
   ...
   bool Save()
   {
      ...
      if(m_group[0].phase==PY_EMPTY && m_group[1].phase==PY_EMPTY &&
         m_group[0].campaignTrimCash==0 && m_group[1].campaignTrimCash==0 && Pending()<0 && m_nonce==0)
      ...
      if(!LoadedStateValid(h))
      { Fault("state fields không hợp lệ; không ghi snapshot mà loader phải từ chối"); return false; }
   ...
   {
      if(!FileSeek(f,(long)sizeof(SPyProtectDisk),SEEK_SET)) return false;
      bool ok=true;
      for(int d=0;d<2;d++) ok=FileReadStruct(f,m_group[d])==sizeof(SPyProtectGroup) && ok;
      if(ArrayResize(m_members,h.members)!=h.members || ArrayResize(m_ops,h.operations)!=h.operations)
         return false;
   ...
   bool LoadedStateValid(const SPyProtectDisk &h) const
   {
      ...
      for(int d=0;d<2;d++)
      {
         if(m_retry[d].rejects<0 || m_retry[d].rejects>8 || m_retry[d].nextEligible<0 ||
            m_retry[d].reconcile!=(m_retry[d].rejects>=8)) return false;
         if(m_retry[d].rejects>0 && (m_retry[d].id==0 || m_retry[d].serial<=0 ||
            m_retry[d].nextEligible==0 ||
            (m_retry[d].kind!=EXEC_CMD_PY_PROTECT_MODIFY &&
             m_retry[d].kind!=EXEC_CMD_PY_PROTECT_CLOSE && m_retry[d].kind!=EXEC_CMD_PY_RH_TRIM))) return false;
      }
      for(int i=0;i<h.members;i++)
         if(m_members[i].dir<0 || m_members[i].dir>1 || m_members[i].id==0) return false;
      for(int i=0;i<h.operations;i++)
      {
         if(m_ops[i].dir<0 || m_ops[i].dir>1 || m_ops[i].id==0 || m_ops[i].ticket==0 ||
            m_ops[i].nonce<=0 || m_ops[i].serial<=0 ||
            !MathIsValidNumber(m_ops[i].beforeLots) || m_ops[i].beforeLots<=0 ||
            !MathIsValidNumber(m_ops[i].requestedLots) || m_ops[i].requestedLots<0 ||
            !MathIsValidNumber(m_ops[i].targetSl) || m_ops[i].targetSl<0) return false;
         if(m_ops[i].kind!=EXEC_CMD_PY_PROTECT_MODIFY &&
            m_ops[i].kind!=EXEC_CMD_PY_PROTECT_CLOSE && m_ops[i].kind!=EXEC_CMD_PY_RH_TRIM) return false;
      }
      return true;
   }
   bool LoadFlatCheckpoint()
   {
      int f=FileOpen(m_file,FILE_BIN|FILE_READ);if(f==INVALID_HANDLE)return false;
      SPyProtectDisk h;ZeroMemory(h);bool ok=FileReadStruct(f,h)==sizeof(SPyProtectDisk);
      uint original=h.semantics;h.semantics=Semantics();ok=ok&&HeaderMatches(h);h.semantics=original;
      long expected=ok?PayloadSize(h):0;uchar bytes[];
      ok=ok&&expected>=0&&(long)FileSize(f)==(long)sizeof(SPyProtectDisk)+expected;
      if(ok)ok=ArrayResize(bytes,(int)expected)==(int)expected&&FileReadArray(f,bytes,0,(int)expected)==(uint)expected&&HashBytes(bytes)==h.checksum;
      if(ok)ok=ReadPayload(f,h,expected)&&LoadedStateValid(h);FileClose(f);
      if(!ok)return false;
      for(int d=0;d<2;d++)if(m_group[d].phase!=PY_EMPTY||m_group[d].campaignTrimCash!=0||m_retry[d].reconcile)return false;
      if(Pending()>=0)return false;m_nonce=h.nonce;return m_nonce>=0;
   }
   bool Load()
   {
   ...
   void CompactRetiredRhOperations()
   {
      if(g_bdArcsReceiptStore==NULL||!g_bdArcsReceiptStore.ReceiptsDurable())return;
      int w=0;
      for(int i=0;i<ArraySize(m_ops);i++)
      {
         SPyProtectOperation op=m_ops[i];bool retire=op.complete&&op.kind==EXEC_CMD_PY_RH_TRIM&&op.coreStartDeal!=m_group[op.dir].coreStartDeal;
         if(retire)
         {
            if(!HistorySelectByPosition(op.id))retire=false;
            else
            {
               int n=HistoryDealsTotal();ulong ids[];if(ArrayResize(ids,n)!=n)retire=false;
               else
               {
                  for(int j=0;j<n;j++)ids[j]=HistoryDealGetTicket(j);bool found=false;
                  for(int j=0;j<n&&retire;j++)
                  {if(ids[j]==0){retire=false;break;}if(!MatchesOperation(ids[j],op))continue;SBDArcsReceipt r;found=true;if(!g_bdArcsReceiptStore.Get(ids[j],r)||!r.pyTrim)retire=false;}
                  retire=retire&&found;
               }
            }
         }
         if(!retire)m_ops[w++]=op;
      }
      if(w!=ArraySize(m_ops)){ArrayResize(m_ops,w);Save();}
   }
...
public:
   ...
   bool Init(CBasketManager *basket,CExecutionLayer *exec,CRecoveryEngine *recovery)
   {
      ...
      if(!Enabled())
      {
         ...
         {if(!LoadFlatCheckpoint()){Log_Error("PYProtect","state chưa flat hoặc không hợp lệ; đối soát trước khi tắt");return false;}}
   ...
   void SettleForIndependentExitT1727() { if(Enabled())SettleOperation(); }
   ...
   bool OperationScope(const ulong id,const long nonce,long &serial,int &direction) const
   {
      for(int i=0;i<ArraySize(m_ops);i++)if(m_ops[i].id==id&&m_ops[i].nonce==nonce)
      {serial=m_ops[i].serial;direction=m_ops[i].kind==EXEC_CMD_PY_RH_TRIM?1-m_ops[i].dir:m_ops[i].dir;return serial>0;}
      return false;
   }
   bool OutcomeConsumed(const ulong positionId,const long nonce) const
   {
      if(m_fault || positionId==0 || nonce<=0 || nonce>m_nonce)return false;
      for(int i=0;i<ArraySize(m_ops);i++)
         if(m_ops[i].nonce==nonce)return m_ops[i].id==positionId && m_ops[i].complete;
      // Nonces are monotonic and persisted with the complete operation set.
      // An absent older intent was settled/rolled back in this checkpoint.
      return true;
   }
