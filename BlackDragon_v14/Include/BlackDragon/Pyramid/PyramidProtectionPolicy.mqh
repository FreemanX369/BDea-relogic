...
{
   long held=reserved<0?0:reserved;
   if(core<=0||core<=held)return 0; // Projected flat always requires RH cleanup, including negative projected reservation.
   if(capPct==0.0)return 9223372036854775807;
   if(capPct!=capPct || capPct<0.0 || capPct>=1.7976931348623157e308)return 0;
   double units=MathFloor((double)(core-held)*capPct/100.0+1e-9);
   if(units>=9223372036854774784.0)return 9223372036854775807;
   return (long)units;
}
